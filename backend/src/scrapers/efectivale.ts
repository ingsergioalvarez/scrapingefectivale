...
/**
 * Extrae el saldo del monedero desde la página actual.
 */
async function extractWalletBalance(page: Page, log: Logger): Promise<number | null> {
  const logLocal: Logger = (m) => log(`[wallet-extract] ${m}`)
  try {
    const text = await page.innerText('.DFC12').catch(() => '')
    if (!text) return null
    const clean = text.replace('$', '').replace(/,/g, '').trim()
    const val = parseFloat(clean)
    if (!isNaN(val)) {
      logLocal(`Saldo Monedero detectado: $${val}`)
      return val
    }
  } catch (e: any) {
    logLocal(`Aviso: No se pudo extraer saldo: ${e.message}`)
  }
  return null
}
...
/**
 * Realiza múltiples recargas en una sola sesión de navegador.
 * Sigue el flujo: Llenar -> Aceptar -> Confirmar -> Verificar -> Siguiente (sin recargar página)
 */
export async function processEfectivaleBulkTopup(args: {
  masterAccount: {
    id: number
    username: string
    passwordEnc: string
  ...
  requests: { cuenta: string, monto: number }[]
  log?: Logger
  debug?: boolean
}): Promise<{ results: { cuenta: string, success: boolean, message: string, error?: string }[] }> {
  const log: Logger = (m) => args.log?.(`[bulk-topup] ${m}`)
  ...
  try {
    ...
    // 2. Navegar a Formulario una sola vez
    await gotoDisperisonTarjeta(page, log)

    // 3. Bucle de Dispersión
    for (const req of args.requests) {
      log(`Procesando cuenta: ${req.cuenta} por $${req.monto}...`)
      try {
        // Asegurar que el campo cuenta esté visible (indica que estamos en la pág correcta)
        const accountInput = page.locator('input[name="cuenta"]').first()
        await accountInput.waitFor({ state: 'visible', timeout: 10000 })

        // Llenar datos
        await accountInput.fill(req.cuenta)
        await page.fill('input[name="montoDispersion"]', req.monto.toString())
        
        log(`Enviando Aceptar para ${req.cuenta}`)
        ...
        // Pantalla de confirmación (Paso 2)
        const confirmText = (await page.innerText('body')).toLowerCase()
        if (confirmText.includes('confirma') || confirmText.includes('aceptar') || confirmText.includes('presione')) {
          log(`Confirmando dispersión para ${req.cuenta}`)
          const confirmBtn = page.locator('input[name="xoAceptar"]').first()
          await Promise.all([
            page.waitForLoadState('domcontentloaded'),
            confirmBtn.click(),
          ])
          // Esperamos a que la franja gris de éxito aparezca
          await page.waitForTimeout(2000)
        }

        // Verificar éxito (Franja gris o texto de éxito)
        const bodyFinal = (await page.innerText('body')).toLowerCase()
        if (
          bodyFinal.includes('éxito') || 
          bodyFinal.includes('exito') || 
          bodyFinal.includes('correctamente') ||
          bodyFinal.includes('realizada')
        ) {
          log(`ÉXITO: ${req.cuenta} dispersada!`)
          finalResults.push({ cuenta: req.cuenta, success: true, message: 'Dispersión completada.' })
          
          // Actualización de Saldo Monedero (Captura del encabezado)
          const currentWallet = await extractWalletBalance(page, log)
          if (currentWallet !== null) await updateWalletBalance(args.masterAccount.id, currentWallet)

          // REGISTRO EN MOVIMIENTOS (Audit Trail)
          try {
             // Traer información completa de la regla y saldos para auditoría
             const allRules = await listTopupRulesMySql();
             const allBalances = await listLatestBalances();
             ...
             console.error("Error al registrar auditoría (éxito):", auditErr);
          }

        } else {
          log(`ERROR: No se pudo confirmar éxito para ${req.cuenta}`)
          finalResults.push({ cuenta: req.cuenta, success: false, error: 'No se detectó confirmación de éxito.' })
          
          // REGISTRO EN MOVIMIENTOS (Audit Trail - Error)
          try {
             const allRules = await listTopupRulesMySql();
             const allBalances = await listLatestBalances();
             const rule = allRules.find(r => r.cuenta === req.cuenta);
             ...
             await createManualGasolinaMovement({
               ...
               monto: req.monto,
               status: 'error',
               error_message: 'No se detectó confirmación de éxito en portal',
               chofer_id: rule?.chofer_id,
               vehiculo_id: rule?.vehiculo_id,
               tarjeta: balance?.tarjeta || rule?.alias,
               codigo_card: rule?.short_code?.toString()
             });
          } catch (auditErr) {
             console.error("Error al registrar auditoría (fallback):", auditErr);
          }
        }

        // PREPARACIÓN PARA SIGUIENTE:
        // En el portal de Efectivale, tras el éxito, a veces hay que dar clic en "Nueva Dispersión"
        // o simplemente el formulario se limpia solo. Según la captura, se queda en la misma página con el formulario listo.
        // Si hay un botón de "Regresar" o similar para limpiar, lo pulsamos.
        const resetBtn = page.locator('input[name="xoCancelar"]').first() // A veces es Cancelar para volver al form limpio
        if (await resetBtn.isVisible().catch(() => false)) {
          // Si vemos que el formulario no está listo, intentamos resetearlo.
          // Pero usualmente tras el mensaje de "Realizada", el portal permite rellenar.
        }

      } catch (e: any) {
        log(`FALLO CRÍTICO en cuenta ${req.cuenta}: ${e.message}`)
        finalResults.push({ cuenta: req.cuenta, success: false, error: e.message })
        
        // Si hubo error de navegación o sesión, intentamos recuperar para la siguiente
...
async function gotoDisperisonTarjeta(page: any, log: Logger) {
  ...
  await closeModalIfPresent(page)
  // Aumentar timeout y reintentar si es necesario
...
/**
 * Consulta solo el saldo del monedero de una cuenta maestra.
 */
export async function fetchEfectivaleWalletBalance(args: {
  masterAccount: {
    id: number
    username: string
    passwordEnc: string
  ...
  const log: Logger = (m) => args.log?.(`[wallet-fetch] ${m}`)
  const headless = args.debug || process.env.EFECTIVALE_DEBUG_HEADFUL === 'true' ? false : config.playwrightHeadless
  ...
  try {
    ...
    const balance = await extractWalletBalance(page, log)
    if (balance !== null) {
      await updateWalletBalance(args.masterAccount.id, balance)
    }
    return balance
  } finally {
    await browser.close()
